...
class CsvReader:
    """ A class which provides functionality for loading csv files into a set of entities """
    def Read(self, file, initializer=None):
        """ Loads a csv file into a dictionary of entities. Values in the first column are used as keys. """
    ...
    def ReadList(self, file, initializer=None):
        """ Loads a csv file into a dictionary of entities. """
        data = list(csv.reader(open(file)))
        if initializer is None:
